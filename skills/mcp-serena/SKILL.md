---
name: mcp-serena
description: Use Serena MCP for code symbol operations - find references, rename symbols, refactor across codebase, understand code structure. Triggers on 'where is X used', 'find references to X', 'rename X to Y', 'what calls X', 'refactor X', 'show X usage', 'track down X'. Also handles Vue/Quasar projects with pattern search fallback for .vue files. Provides 40-70% token savings over reading entire files.
---
